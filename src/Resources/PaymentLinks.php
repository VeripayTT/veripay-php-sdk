...
class PaymentLinks
{
    ...
    {
        return $this->client->request('POST', '/api/v1/payment-links', [
            'json' => $data
        ]);
    }
    ...
    {
        return $this->client->request('GET', '/api/v1/payment-links');
    }

    public function get(string $linkId): array
    {
        return $this->client->request('GET', "/api/v1/payment-links/{$linkId}");
    }
}
